...
void map3(){
    //Move to main()
    char** global = taskOneFilter("tester.txt", "CTask1.txt");
    int index[13];
    pthread_t thread[13]; 
    //Create a global array
    for(int i = 0; i < 13; i++){
        //Last NULL on create is the parameter to pass in
        if(pthread_create(&index[i], NULL, &reduce3, NULL)!=0){
            //Error occured
        }
        //LAST null on join is the return value
        if(pthread_join(index[i], NULL)!= 0){
            //Error occured
        }
    }
